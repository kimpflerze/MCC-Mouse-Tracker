...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`novackgm`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `statistics` AS select (select uuid()) AS `id`,(select coalesce(count(`male`.`id`),0) from `breeding_male` `male`) AS `breeding_males`,(select coalesce(count(`cages`.`id`),0) from (`breeding_cage` `cages` join `generic_cage` `generic` on((`cages`.`id` = `generic`.`id`))) where (`generic`.`active_flag` = 1)) AS `breeding_cages`,(select coalesce(count(`cages`.`id`),0) from (`breeding_cage` `cages` join `generic_cage` `generic` on((`cages`.`id` = `generic`.`id`))) where ((`generic`.`active_flag` = 1) and (`cages`.`litter_dob` is not null))) AS `num_litters`,(select coalesce(count(`cages`.`id`),0) from (`selling_cage` `cages` join `generic_cage` `generic` on((`cages`.`id` = `generic`.`id`))) where (`generic`.`active_flag` = 1)) AS `selling_cages`,(select coalesce(sum(`cages`.`num_mice`),0) from (`selling_cage` `cages` join `generic_cage` `generic` on((`cages`.`id` = `generic`.`id`))) where (`generic`.`active_flag` = 1)) AS `stock_mice`,(select coalesce(sum(`cages`.`num_mice`),0) from (`selling_cage` `cages` join `generic_cage` `generic` on((`cages`.`id` = `generic`.`id`))) where ((`generic`.`active_flag` = 1) and (`cages`.`gender_flag` = 1))) AS `stock_males`,(select coalesce(sum(`cages`.`num_mice`),0) from (`selling_cage` `cages` join `generic_cage` `generic` on((`cages`.`id` = `generic`.`id`))) where ((`generic`.`active_flag` = 1) and (`cages`.`gender_flag` = 0))) AS `stock_females`,(select coalesce(sum(`o`.`number_of_male_mice`),0) from `orders` `o` where (`o`.`active` = 1)) AS `males_ordered`,(select coalesce(sum(`o`.`number_of_female_mice`),0) from `orders` `o` where (`o`.`active` = 1)) AS `females_ordered`,(select coalesce(count(`o`.`id`),0) from `orders` `o`) AS `orders`,(select coalesce(count(`a`.`id`),0) from `alerts` `a`) AS `alerts` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
BEGIN
	...
	WHILE i <= cage_count DO
		...
		INSERT INTO alerts VALUES (null, 3, (SELECT current_cage_id FROM IndexedCagesOldFemale WHERE i = row_number), (SELECT alert_date FROM IndexedCagesOldFemale WHERE i = row_number), 0);
...
BEGIN
	...
	WHILE i <= cage_count DO
        ...
        #Insert into the alerts table
		INSERT INTO alerts VALUES (null, 2, (SELECT id FROM IndexedCagesOldMale WHERE i = row_number), (SELECT alert_date FROM IndexedCagesOldMale WHERE i = row_number), 0);
...
BEGIN
	...
	WHILE i <= cage_count DO
        ...
        #Insert into the alerts table
		INSERT INTO alerts VALUES (null, 1, (SELECT id FROM IndexedCagesToWean WHERE i = row_number), (SELECT alert_date FROM IndexedCagesToWean WHERE i = row_number), 0);
...
-- Dump completed on 2018-05-03 16:32:50
